#include "Biped\bpd\String.h"

BPD_BEGIN
// ---- COPY ---- //
void String::strCopy(char** dest, const char* str) {
	BPD_SAFE_ARRAY_DELETE(*dest);
...
bool String::operator==(const char * other) {
	if (this->length() != length(other)) return false;
	for (int i = 0; i < this->length(); i++) {
		if (this->data[i] != other[i]) return false;
	} return true;
}

bool operator==(const char * str, String & other) {
	if (other.length() != other.length(str)) return false;
	for (int i = 0; i < other.length(); i++) {
		if (other.data[i] != str[i]) return false;
	} return true;
}

// ---- CONSTRUCTOR ---- //
String::String() {
	BPD_SAFE_ARRAY_DELETE(data);
}
...
void String::replace(char splitChar, char* value) {
	...
	BPD_SAFE_ARRAY_DELETE(str_first);
	BPD_SAFE_ARRAY_DELETE(str_last);
}
